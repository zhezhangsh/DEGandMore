...
if (nrow(outlier) == 0) lns<-'No outliers found' else if (nrow(outlier) < 3) lns<-'No enough outliers for clustering analysis' else {
...
DEGs having higher or lower expression in each outliers were mapped to a collection of predefined geneset 

```{r gse, include=FALSE}
path.gse<-paste(yml$output, 'gse', sep='/');
if (!file.exists(path.gse)) dir.create(path.gse, recursive = TRUE);
if (nrow(outliers) == 0) lns<-'No outliers found' else {
  gse<-lapply(names(degs), function(nm) {
    cat(nm, '\n'); 
  ...
  fn.hi<-sapply(gse, function(g) g[[1]]$file[[gset.src]]);
  fn.lo<-sapply(gse, function(g) g[[2]]$file[[gset.src]]);
  
  fn.hi<-sub(paste(yml$output, '/', sep=''), '', fn.hi);
  fn.lo<-sub(paste(yml$output, '/', sep=''), '', fn.lo);
  
  n.hi<-sapply(gse, function(g) sapply(g[[1]]$formatted[gset.src], nrow)); 
  n.lo<-sapply(gse, function(g) sapply(g[[2]]$formatted[gset.sce], nrow)); 
