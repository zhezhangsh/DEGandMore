...
path.gsea<-paste(path.output, 'gsea', names(degs), sep='/');
sapply(path.gsea, function(path.gsea) if (!file.exists(path.gsea)) dir.create(path.gsea, recursive = TRUE))->x;
names(gse)<-names(degs);
saveRDS(gse, paste(path.output, 'gsea', 'gsea_full.rds', sep='/'));

gsea.table<-lapply(gse, function(gse) {
  g<-gse$stat;
...
# Formatted tables
gsea.formatted<-lapply(gsea.table, function(t) {
