...
knitr::opts_chunk$set(eval=FALSE, dpi=300, fig.pos="H", fig.width=8, fig.height=6, echo=FALSE, warning=FALSE, message=FALSE);
...
path.ora<-paste(path, 'ORA', names(degs), sep='/');
sapply(path.ora, function(path.ora) if (!file.exists(path.ora)) dir.create(path.ora, recursive = TRUE))->x;
...
# Formatted tables
ora.formatted<-lapply(ora.table, function(t) {
  an<-geneset[[1]][rownames(t), ];
  t$Term<-awsomics::AddHref(t$Term, an[, 'URL']);
  t$Total<-awsomics::AddHref(t$Total, paste('./term/term_', rownames(t), '_total.html', sep=''));
  t$Within<-awsomics::AddHref(t$Within, paste('./term/term_', rownames(t), '_within.html', sep=''));
  rownames(t)<-1:nrow(t);
  awsomics::FormatNumeric(t);
}); 
...
cnm<-c('Sample', colnames(anno), colnames(up));
up.tbl<-data.frame(rep('Figure', nrow(up)), anno[rownames(up.tbl), , drop=FALSE], up);
colnames(up.tbl)<-cnm;
up.tbl$Samples<-awsomics::AddHref(up.tbl$Samples, paste('./bars/', rownames(up), '.pdf', sep='')); 
up.formatted<-GeneList2Datatable(up.tbl, paste(pth.deg1, 'index.html', sep='/'), col.symbol=1, genome=genome, 
              title=paste('Genes with higher expression in', g2.name), extra.url=extra.url);

dn.tbl<-data.frame(rep('Figure', nrow(dn)), anno[rownames(dn), , drop=FALSE], dn);
colnames(dn.tbl)<-cnm;
dn.tbl$Samples<-awsomics::AddHref(dn.tbl$Samples, paste('./bars/', rownames(dn), '.pdf', sep='')); 
colnames(dn.tbl)<-cnm;
dn.formatted<-GeneList2Datatable(dn.tbl, paste(pth.deg2, 'index.html', sep='/'), col.symbol=1, genome=genome, 
              title=paste('Genes with lower expression in', g2.name), extra.url=extra.url);

### write Excel file
# format outputs
f0<-list(id.name='Gene_ID', format=data.frame(ind=ncol(anno)+(1:10), fmt=c(rep('0.000', 4), '0', '0.0E+0', '0.0E+0', '0', '0.0E+0', '0.0E+0'))); # format: group comparison, full list
f1<-list(id.name='Gene_ID', format=data.frame(ind=ncol(anno)+(1:7), fmt=c('0', rep('0.000', 4), '0.0E+0', '0.0E+0'))); # format: group comparison
f2<-list(row.names=FALSE, format=data.frame(ind=3:7, fmt=c('0', '0.00', '0.00', '0.0E+0', '0.0E+0'))); # format: DAVID output

# create spreadsheet of Excel file
full<-cbind(out[[1]][, c(1, 6, 7)], out[[2]][, c(1, 6, 7)]);
colnames(full)<-paste(colnames(full), rep(names(out)[1:2], each=ncol(full)/2), sep='_');
full<-cbind(out[[3]][, 2:5], full);
full<-cbind(anno[rownames(full), ], full);
xls<-list(full, cbind(anno[rownames(up),], up), cbind(anno[rownames(dn), ], dn), dv.up$formatted, dv.dn$formatted);
xls[[4]][, 4:7]<-sapply(4:7, function(i) as.numeric(as.vector(xls[[4]][, i])));
xls[[4]][, 'FDR']<-xls[[4]][,'FDR']/100;
xls[[5]][, 4:7]<-sapply(4:7, function(i) as.numeric(as.vector(xls[[5]][, i])));
xls[[5]][, 'FDR']<-xls[[5]][,'FDR']/100;

#xls<-list(out, anno[rownames(, dn, dv.up=dv[[1]], dv.dn=dv[[2]]);
settings<-list(f0, f1, f1, f2, f2);
names(xls)<-c('Complete List', 
  paste('Top', nrow(up), ', higher in ', g2.name, sep=''), 
	paste('Top', nrow(dn), ', lower in ', g2.name, sep=''),
	paste('DAVID, higher in ', g2.name, sep=''),
	paste('DAVID, lower in ', g2.name, sep='')
);
