...
<span style="color:darkblue; font-family:Georgia; font-size:1.5em;"> Click a number to see table, or read the rest of this page for details: </span>
 ...
 - Differential gene expression ([All genes](DEG/all_genes.html))
   - [`r nrow(degs[[1]])` genes](`r paste('DEG', names(degs)[1], 'index.html', sep='/')`) with increased expression in `r g1.name`
   - [`r nrow(degs[[2]])` genes](`r paste('DEG', names(degs)[2], 'index.html', sep='/')`) with decreased expression in `r g1.name`
 - Download
   - [Excel](`r paste(path0, '.xlsx', sep='')`) file with multiple statistical tables can be downloaded from [here](./`r paste(path0, '.xlsx', sep='')`)
