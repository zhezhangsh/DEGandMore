...
ora.wrapped<-lapply(names(ora.table), function(nm) {
  ...
  WrapGSE(ora.table[[nm]], geneset$meta, TRUE, pth); 
});
...
input.gsea<-inputs$gsea;
gsea.yml<-input.gsea$template;
if (input.gsea$remote) gsea.yml<-yaml.load(RCurl::getURL(input.gsea$template)) else gsea.yml<-yaml.load_file(input.gsea$template)
