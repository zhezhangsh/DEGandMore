...
##########################################################################
if (!exists('inputs')) stop("Required variable <inputs> not loaded.\n");
##########################################################################
...
#file.copy(path.input, paste(path, 'input.rds', sep='/'));
zip(path, path, zip='zip');
file.rename(paste(path, '.zip', sep=''), paste(path, '/', path0, '.zip', sep=''));
```

***
# Results at a glance
  
<span style="color:red; font-family:Georgia; font-size:1.5em;">Click on numbers to browse:</span>
...
Click link to get full list of DEGs:
 ...
 > [Lower in `r g2.name`](./DEG/`r names(degs)[1]`/index.html)
...
GSEA is a different category of analyses applied to gene sets. Instead of using selected top DEGs like **ORA**, GSEA takes into account all genes in the data set after they were ranked by a statistical test such as **Rank Product** test. The original [GSEA](http://www.broadinstitute.org/gsea/index.jsp) was developed by Broad Institute that is usually run as a stand-alone program. We used a similar method call [GAGE](http://www.biomedcentral.com/1471-2105/10/161) in this analysis. By default, the normalized rand products of all genes were used to make colored pathway plots. 
...
```{r bioconductor, eval=TRUE, include=TRUE}
...
An example of input data can be download from [here](https://github.com/zhezhangsh/DEGandMore/blob/master/examples/DeReport/DeReport_Inputs.rds?raw=true)
