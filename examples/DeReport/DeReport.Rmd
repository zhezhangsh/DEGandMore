...
fn.yaml<-"/nas/is1/zhangz/projects/falk/2015-01_Worm_Antioxidant_Drugs/R/pairwise/DeReport.yml"; # comment it out if running through knitr::knit function
...
gsea.bic2gs<-lapply(gsea.bic, function(bic) lapply(bic[[2]], rownames));
...
gsea.bic.formatted<-lapply(names(gsea.bic), function(nm) {
  ...
  t<-awsomics::FormatNumeric(gsea.bic[[nm]]$summary);
  t$ID<-as.vector(t$ID); 
...
capture.output(kegg<-pathview(kegg.dff, pathway.id=kegg.id, species = GetGenomeAlias(genome, "code"), 
                              low = list(gene = "blue"), kegg.dir=yml$input$gsea$kegg))->x; 
fn.kegg<-paste(kegg.id, '.pathview.png', sep=''); 
file.rename(fn.kegg, paste(path.kegg, fn.kegg, sep='/')); 

kegg.ind<-FormatNumeric(kegg.stat[, -1]);
kegg.ind$Gene_set<- AddHref(kegg.ind$Gene_set, fn.kegg); 
...
## Input summary
 ...
 - **GSEA gene set files:**
`r paste(paste('      - **', names(yml$input$gsea$gmt), ':** ', unlist(yml$input$gsea$gmt), sep=''), collapse='\n')`
...
ex.kegg<- kegg.stat[abs(kegg.stat$NES)==max(abs(kegg.stat$NES)), ];
...
##References
 ...
 - **GSEA:** Subramanian A et al. 2005 _Gene set enrichment analysis: A knowledge-based approach for interpreting genome-wide expression profiles_ PNAS. [Home page](gsea rna-seq)
