...
### More about individual clusters
...
fns<-sapply(1:length(cl), function(i) {
  ...
  fn<-paste(name, '.html', sep=''); 
  if (yml$input$remote) {
    md<-knit(text=yml$input$template); 
    writeLines(knit2html(text=md), fn)
  } else {
    md<-knit(yml$input$template); 
    knit2html(md, fn); 
  } 
  paste(path, fn, sep='/'); 
}); 
...
Click names to see more detaied information about individual clusters
