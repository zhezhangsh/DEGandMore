...
fn.yaml<-"ClReport_WormAntioxidant.yml"; # comment it out if running through knitr::knit function
...
PlotColoredBlock(d, num.breaks = 127, groups=split(colnames(d), colnames(d)), key='Group average'); 
...
PlotColoredBlock(d, num.breaks = 127, groups=split(colnames(d), colnames(d)), key='Expression level'); 
abline(h=c(nrow(d), nrow(d)-cumsum(sapply(cl, length)), lwd=1)); 
