...
`r nrow(e)` genes selected by the last step were clustering using the following steps:
...
while(flag) {
  ...
  if (r>prms$cluster$merge$corr & p<prms$cluster$merge$p & length(cl)>length(grp)) {
...
The initial analysis identified `r length(cl)` gene clusters using just selected significant genes. The sizes of the clusters ranged between `r min(n)` and `r max(n)`. 
...
PlotColoredBlock(sortColumn(ms, grp), key='Average expression', groups=grp); 
```
...
#### Trace the change of cluster sizes during reclustering

**Figure** This plot shows the change of cluster sizes after each reclustering cycle. 
...
Result tables:
  ...
  - Normalized expressiond data of clustered genes
    - Click [here](`r paste(yml$output, 'clustered_data.html', sep='/')`) to view online
    - Click [here](`r paste(yml$output, 'clustered_data.csv', sep='/')`) to download table
  - Statistical results of clustered genes
    - Click [here](`r paste(yml$output, 'clustered_stat.html', sep='/')`) to view online
    - Click [here](`r paste(yml$output, 'clustered_stat.csv', sep='/')`) to download table

## Plot clusters

**Figure** Plot the group means and standard errors of all cluster together. 

```{r plot_means, include=TRUE, out.width='800px'}
ms<-sapply(cl, function(cl) colMeans(expr[cl, ]));
...
PlotSeries(m, se, c('Average expression', '')); 
```

**Figure** Heatmap of all clustered genes

```{r heatmap, include=TRUE, out.width='900px', out.height='1200px'}
...
## More details about individual clusters
...
Click names to see more detaied information about each cluster
