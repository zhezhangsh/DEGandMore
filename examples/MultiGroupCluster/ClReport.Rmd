...
fns<-sapply(1:length(cl), function(i) {
  ...
  fn.html<-paste(yml$output, name, paste(name, '.html', sep=''), sep='/'); 
  
  writeLines(rmarkdown::render(fn.temp, output_file=fn.html), "render_wrapper.R");
  devtools::clean_source("render_wrapper.R", quiet = TRUE); # Nested render confuses RStudio, use this trick to clean it up
...
```{r wrap_up, include=FALSE}
x<-strsplit(yml$output, '/')[[1]]; 
fn.zip<-paste(x[length(x)], '.zip', sep='');
if (file.exists(paste(yml$output, fn.zip, sep='/'))) file.remove(paste(yml$output, fn.zip, sep='/')); 
zip(fn.zip, yml$output, zip='zip'); 
file.rename(fn.zip, paste(yml$output, fn.zip, sep='/')); 
```

# Download all

Click [here](fn.zip) download all results as a .zip file
