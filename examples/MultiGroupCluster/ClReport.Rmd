...
fns<-sapply(1:length(cl), function(i) {
  ...
  if (yml$input$remote) {
    md<-knit(text=yml$input$subtemplate); 
    writeLines(knit2html(text=md), fn)
  } else {
    md<-knit(yml$input$subtemplate); 
    knit2html(md, fn.html); 
  } 
  paste(path, fn.html, sep='/'); 
}); 
...
fn.index<-paste(getwd(), 'ClReport.html', sep='/');
fn.out<-paste(yml$output, 'index.html', sep='/');
if(file.exists(fn.index)) file.copy(fn.index, fn.out); 

x<-strsplit(yml$output, '/')[[1]]; 
fn.zip<-x[length(x)];
fn.zip.fn<-paste(fn.zip, paste(fn.zip, '.zip', sep=''), sep='/');
setwd(paste(x[-length(x)], collapse='/')); 
zip(paste(fn.zip, '.zip', sep=''), fn.zip, zip='zip'); 
if (file.exists(fn.zip.fn)) file.remove(fn.zip.fn); 
file.rename(paste(fn.zip, '.zip', sep=''), fn.zip.fn); 
...
Click [here](paste(fn.zip, '.zip', sep='')) download all results as a .zip file
